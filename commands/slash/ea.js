  ...
  async execute(interaction) {
    ...
    collector.on('collect', async i => {
      if (!i.member.roles.cache.some(r => allowedRoleIds.includes(r.id))) {
        return i.reply({
          embeds: [new EmbedBuilder().setDescription('You do not have the required role.').setColor(embedColor).setThumbnail(GREENVILLE_FOOTER_ICON_URL).setFooter(greenvilleFooter(interaction))],
          ephemeral: true
        });
      }
      ...
      if (!startup) {
        return i.reply({
          embeds: [new EmbedBuilder().setDescription('Startup message not found. Please ask a host to initiate one.').setColor(embedColor).setThumbnail(GREENVILLE_FOOTER_ICON_URL).setFooter(greenvilleFooter(interaction))],
          ephemeral: true
        });
      }
      ...
      if (!startupMsg) {
        return i.reply({
          embeds: [new EmbedBuilder().setDescription('Startup message no longer exists.').setColor(embedColor).setThumbnail(GREENVILLE_FOOTER_ICON_URL).setFooter(greenvilleFooter(interaction))],
          ephemeral: true
        });
      }
      ...
      await i.reply({
        embeds: [new EmbedBuilder().setDescription(`Click here to join the session:\n${sessionLink}`).setColor(embedColor).setThumbnail(GREENVILLE_FOOTER_ICON_URL).setFooter(greenvilleFooter(interaction))],
        ephemeral: true
      });
