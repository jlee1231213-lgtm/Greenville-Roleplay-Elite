  ...
  async execute(interaction) {
    ...
    if (!settings) {
      return interaction.editReply({
        embeds: [new EmbedBuilder().setDescription('Settings not found').setColor('#4C7C58').setThumbnail(GREENVILLE_FOOTER_ICON_URL).setFooter(greenvilleFooter(interaction))],
      });
    }
    ...
    if (!staffRoleId || !interaction.member.roles.cache.has(staffRoleId)) {
      return interaction.editReply({
        embeds: [new EmbedBuilder().setDescription('You must have the Staff role').setColor('#4C7C58').setThumbnail(GREENVILLE_FOOTER_ICON_URL).setFooter(greenvilleFooter(interaction))],
      });
    }
    ...
    if (startupCooldownUntil && startupCooldownUntil > Date.now()) {
      return interaction.editReply({
        embeds: [new EmbedBuilder().setDescription(`You can use /startup again in ${formatCooldown(startupCooldownUntil - Date.now())}.`).setColor('#4C7C58').setThumbnail(GREENVILLE_FOOTER_ICON_URL).setFooter(greenvilleFooter(interaction))],
      });
    }
