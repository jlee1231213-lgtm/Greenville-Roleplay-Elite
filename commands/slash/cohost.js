  ...
  async execute(interaction) {
    ...
    if (!allowedRoleId || !interaction.member.roles.cache.has(allowedRoleId)) {
      return interaction.editReply({
        embeds: [new EmbedBuilder().setDescription('No permission').setColor(embedColor).setThumbnail(GREENVILLE_FOOTER_ICON_URL).setFooter(greenvilleFooter(interaction))],
      });
    }
    ...
    await interaction.editReply({ content: 'Cohost registered successfully.', flags: MessageFlags.Ephemeral });
  }
};
