    ...
    if (settings) {
      ...
      if (staffRoleId && !interaction.member.roles.cache.has(staffRoleId)) {
        return interaction.editReply({
          embeds: [new EmbedBuilder().setDescription('You must have the Staff role to use this command.').setColor('#4C7C58').setThumbnail(GREENVILLE_FOOTER_ICON_URL).setFooter(greenvilleFooter(interaction))],
        });
      }
    }
